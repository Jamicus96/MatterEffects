...
int main(int argc, char *argv[]) {
    // Read in arguments
    std::vector<std::vector<double> > input_data = read_args(argc, argv, 18);
    std::cout << "Reading last 3 args" << std::endl;
    unsigned int init_flavour = atoi(argv[13]); // 0=e, 1=mu, 2=tau
    unsigned int final_flavour = atoi(argv[14]); // 0=e, 1=mu, 2=tau
    int anti = atoi(argv[15]); // -1 = antineutrino, 1 = neutrino
    std::string output_file_address = argv[16];
    bool print_probs = atoi(argv[17]);
    ...
    long double time_P, time_P_vac, time_P_globes, time_P_vac_globes, time_P_specific;
    clock_t c_start, c_end;
    unsigned int N = input_data.size();

    double P_temp;
    // Convert flavours for GLoBES
    unsigned int glb_init_flavour = init_flavour + 1;
    unsigned int glb_final_flavour = final_flavour + 1;
    ...
    // Run oscillation functions
    c_start = clock();
    for (unsigned int i = 0; i < N; ++i) {
        P_temp = Oscillation_Prob(consts, input_data.at(i).at(0), input_data.at(i).at(1), input_data.at(i).at(2), init_flavour, final_flavour, anti);
        // P.push_back(P_temp);
    }
    c_end = clock();
    std::cout << "c_start = " << c_start << ", c_end = " << c_end << std::endl;
    time_P = (long double)(c_end - c_start) / (long double)CLOCKS_PER_SEC;
    c_start = clock();
    for (unsigned int i = 0; i < N; ++i) {
        P_temp = Oscillation_Prob_Vac(U_PMNS, input_data.at(i).at(0), input_data.at(i).at(1), init_flavour, final_flavour, anti);
        // Record info
        // P_vac.push_back(P_temp);
    }
    c_end = clock();
    time_P_vac = (long double)(c_end - c_start) / (long double)CLOCKS_PER_SEC;
    c_start = clock();
    for (unsigned int i = 0; i < N; ++i) {
        P_temp = glbConstantDensityProbability(glb_init_flavour, glb_final_flavour, anti, input_data.at(i).at(1), input_data.at(i).at(0), input_data.at(i).at(2));  // flavours + 1 to mine, and energy in GeV
        // Record info
        // P_globes.push_back(P_temp);
    }
    c_end = clock();
    time_P_globes = (long double)(c_end - c_start) / (long double)CLOCKS_PER_SEC;
    c_start = clock();
    for (unsigned int i = 0; i < N; ++i) {
        P_temp = glbVacuumProbability(glb_init_flavour, glb_final_flavour, anti, input_data.at(i).at(1), input_data.at(i).at(0));  // flavours + 1 to mine, and energy in GeV
        c_end = clock();
        // Record info
        // P_vac_globes.push_back(P_temp);
    }
    c_end = clock();
    time_P_vac_globes = (long double)(c_end - c_start) / (long double)CLOCKS_PER_SEC;
    ...
    if (init_flavour == 0 && final_flavour == 0 && anti == -1) {
        ...
        c_start = clock();
        for (unsigned int i = 0; i < N; ++i) {
            P_temp = anti_e_e_Survival_Prob(consts_e_e, input_data.at(i).at(2), input_data.at(i).at(1), input_data.at(i).at(0));
            // Record info
            // P_specific.push_back(P_temp);
        }
        c_end = clock();
        time_P_specific = (long double)(c_end - c_start) / (long double)CLOCKS_PER_SEC;
    } else if (init_flavour == 1 && anti == 1) {
        if (final_flavour == 1) {
            // Run oscillation function
            c_start = clock();
            for (unsigned int i = 0; i < N; ++i) {
                P_temp = mu_mu_Survival_Prob(consts_m_m, input_data.at(i).at(2), input_data.at(i).at(1), input_data.at(i).at(0));
                // Record info
                // P_specific.push_back(P_temp);
            }
            c_end = clock();
            ...
            c_start = clock();
            for (unsigned int i = 0; i < N; ++i) {
                P_temp = mu_e_Transition_Prob(consts_m_e, input_data.at(i).at(2), input_data.at(i).at(1), input_data.at(i).at(0));
                // Record info
                // P_specific.push_back(P_temp);
            }
            c_end = clock();
    ...
    if (print_probs) {
        ...
        for (unsigned int i = 0; i < N; ++i) {
            ...
            if (P.size() > 0) {datafile << " " << P.at(i);} else {datafile << " -";}
            if (P_vac.size() > 0) {datafile << " " << P_vac.at(i);} else {datafile << " -";}
            if (P_globes.size() > 0) {datafile << " " << P_globes.at(i);} else {datafile << " -";}
            if (P_vac_globes.size() > 0) {datafile << " " << P_vac_globes.at(i);} else {datafile << " -";}  
            if (P_specific.size() > 0) {datafile << " " << P_specific.at(i);} else {datafile << " -";}
            datafile << std::endl;
        }
    }
    datafile << "- - - - - - " << time_P << " " << time_P_vac << " " << time_P_globes << " " << time_P_vac_globes << " " << time_P_specific << std::endl;
...
std::vector<std::vector<double> > read_args(int argc, char *argv[], unsigned int n_args_wanted) {
    ...
    if (L_N <= 0 || E_N <= 0 || rho_N <= 0) {
        std::cout << "Number of datapoints E_N, rho_N and L_N must be greater than 0, not " << E_N << ", " << rho_N << " and " << L_N << std::endl;
        exit(1);
    }
    ...
    if (L_N == 1) {
        if (L_max == L_min) {
            L_steps.push_back(L_min);
        } else {
            std::cout << "L_min and L_max must be equal if L_step = 1, not L_min = " << L_min << ", and L_max = " << L_max << std::endl;
            exit(1);
        }
    } else {
    ...
    if (E_N == 1) {
        if (E_max == E_min) {
            E_steps.push_back(E_min);
        } else {
            std::cout << "E_min and E_max must be equal if E_step = 1, not E_min = " << E_min << ", and E_max = " << E_max << std::endl;
            exit(1);
        }
    } else {
    ...
    if (rho_N == 1) {
        if (rho_max == rho_min) {
            rho_steps.push_back(rho_min);
        } else {
            std::cout << "rho_min and rho_max must be equal if rho_step = 1, not rho_min = " << rho_min << ", and rho_max = " << rho_max << std::endl;
            exit(1);
        }
    } else {
